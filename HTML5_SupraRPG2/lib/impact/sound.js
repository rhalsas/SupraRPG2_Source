...
ig.Music = ig.Class.extend({
	...
	fadeOutAndPlay: function (time, name) {
	    if (!this.currentTrack) { return; }
	    this.nextAfterFade = name;
	...
	fadeInAndPlay: function (time,name) {
	    if (!this.currentTrack) { return; }
	    ...
	    clearInterval(this._fadeInterval);
	    this.fadeTimer = new ig.Timer(time);
	    this._fadeToInterval = setInterval(this._fadeInStep.bind(this), 50);
	},
	...
	_fadeInStep: function () {
	    ...
	    if (v === 1) {
	        ...
	        clearInterval(this._fadeToInterval);
	    }
	    else { 
	...
	_fadeStep: function() {
		...
		if( v <= 0.01 ) {
		    ...
		    if (this.nextAfterFade !== '') {
		       this.setVolume(0);


		        this.fadeInAndPlay(0.5, this.nextAfterFade);
		        this.nextAfterFade = '';
		    }
			clearInterval(this._fadeInterval);
		}
...
ig.Sound.use = [ig.Sound.FORMAT.M4A, ig.Sound.FORMAT.WAV, ig.Sound.FORMAT.OGG];
ig.Sound.channels = 4;
