...
ig.Game = ig.Class.extend({
	...
	loadLevel: function( data ) {
		...
		for( var i = 0; i < data.entities.length; i++ ) {
		    ...
		    for (var z = 0; z < ig.game.destroyedEntitiesArray.length; z++) {
		        if (ent.type === "EntityTreasureBig" &&
                    ig.game.destroyedEntitiesArray[z].x === ent.x &&
                    ig.game.destroyedEntitiesArray[z].y === ent.y) {
		    ...
		    if (cannotSpawn === false) { this.spawnEntity(ent.type, ent.x, ent.y, ent.settings); }
		...
		ig.game.itemLevel = ig.game.checkSpecialLevel(data);
        
	},

	a_loadLevel: function( data , callback) {
	    this.screen = { x: 0, y: 0 };
